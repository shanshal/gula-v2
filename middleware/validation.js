...
const validateSurveyCreation = (req, res, next) => {
  try {
    ...
    if (!isString(name)) {
      errors.push({ field: 'name', message: 'Survey name is required and must be a non-empty string' });
    }
...
const validateQuestion = (question, index) => {
  ...
  if (!isString(questionText)) {
    errors.push({
      field: `questions[${index}].text`,
      message: `Question ${questionNumber}: question_text or text field is required and must be a non-empty string`
    });
  }

  // Check for required type field (support both formats)
  const questionType = question.type || question.question_type;
  const validTypes = ['radio', 'checkbox', 'text', 'number', 'select', 'textarea', 'single_choice', 'multiple_choice'];
      ...
      question.options.forEach((option, optionIndex) => {
        if (!isString(option)) {
          errors.push({
            ...
            message: `Question ${questionNumber}, option ${optionIndex + 1}: must be a non-empty string`
          });
        }
      });
    }
...
const validateAnswerSubmission = (req, res, next) => {
  try {
    ...
    if (isNaN(userIdNum) || userIdNum < 1) {
      errors.push({ field: 'userId', message: 'Valid user ID is required in URL (positive integer)' });
    }

    if (isNaN(surveyIdNum) || surveyIdNum < 1) {
      errors.push({ field: 'surveyId', message: 'Valid survey ID is required in URL (positive integer)' });
    }
...
const validateSingleAnswer = (req, res, next) => {
  try {
    ...
    // Required fields
    if (!isNumber(user_id) || user_id < 1) {
      errors.push({ field: 'user_id', message: 'Valid user_id is required (positive integer)' });
    }
